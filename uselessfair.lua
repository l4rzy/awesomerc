...
local function size_correction(c, geometry, useless_gap)
    geometry.width  = geometry.width - 3 * useless_gap +1 
    geometry.height = geometry.height - 3 * useless_gap -1
	--geometry.x = geometry.x 
    geometry.y = geometry.y + useless_gap / 2 

	if (geometry.width < 700) then	
		--geometry.x = geometry.x  
		geometry.width = geometry.width + 2 
	end	
end
...
local function fair(p, orientation)
    ...
    -- Theme vars
    local useless_gap = 3
    local global_border = beautiful.global_border_width or 1
    ...
    wa.height = wa.height - 2 * global_border 
    wa.width  = wa.width - 2 * global_border
